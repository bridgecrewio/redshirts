...
export default class AzureDevOps extends Command {
    ...
    static flags = {
        ...
        orgs: Flags.string({
            description:
                'Org names for which to fetch repos. Use the --repos and / or --projects options to add additional specific repos on top of those in the specified org(s). Use the --skip-repos and / or --skip-projects options to exclude individual repos or projects that are a part of these org(s).',
            required: false,
            helpGroup: HelpGroup.REPO_SPEC,
        }),
        projects: Flags.string({
            description:
                'Project names for which to fetch repos. Use the --repos option to add additional specific repos on top of those in the specified project(s). Use the --skip-repos option to exclude individual repos that are a part of these project(s).',
            required: false,
            helpGroup: HelpGroup.REPO_SPEC,
        }),
        'skip-projects': Flags.string({
            description:
                'Project names for which to skip fetching repos. Use this option to skip projects that are part of the specified orgs. If the same project is included in both --projects and --skip-projects, it is skipped. If a repo from a skipped project is included in --repos, it is also still skipped.',
            required: false,
            helpGroup: HelpGroup.REPO_SPEC,
        }),
        ...vcsFlags,
