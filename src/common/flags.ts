...
export const commonFlags = {
    output: Flags.enum({
        description:
            "Output format for displaying data. Defaults to 'summary', which is a console-friendly tabular output. To see full committer details, instead of just the unique count (total and per repo), use 'json'.",
        char: 'o',
    ...
    sort: Flags.enum({
        description:
            'The output field on which to sort for CSV or console output: alphabetically by repo fully qualified name, or by descending contributor count (ignored for JSON)',
...
export const vcsServerFlags = {
    hostname: Flags.string({
        description:
            'The hostname of your server, e.g. `git.mycompany.com`. Do not include the port and protocol here (see --port and --protocol).',
        char: 'h',
        required: true,
        helpGroup: HelpGroup.CONNECTION,
    }),
    ...
    protocol: Flags.enum({
        description:
            'Protocol for your server, https (default) or http. Affects the default port value if you do not specify a port. You must specify a protocol here, not in the hostname.',
...
export const throttlingFlags = {
    'requests-per-hour': Flags.integer({
        description:
            'The maximum number of requests to the server per hour. Requests will be throttled and spread to achieve this limit. For example, if you specify a value of 60 here, one request per minute will be submitted. Use this to adjust the throttling of the API calls in the event that the rate limit is being consumed by other sources simultaneously.',
        required: false,
        default: 1000,
        helpGroup: HelpGroup.CONNECTION,
    }),
...
export const getThrottlingFlag = (defaultRequestsPerHour: number): { 'requests-per-hour': OptionFlag<number> } => {
    return {
        'requests-per-hour': Flags.integer({
            description:
                'The maximum number of requests to the server per hour. Requests will be throttled and spread to achieve this limit. For example, if you specify a value of 3600 here, approximately one request per second will be submitted. Use this to adjust the throttling of the API calls in the event that the rate limit is being consumed by other sources simultaneously. If you are running in a self-hosted server environment without API rate limits, you can also set this to a very high number to effectively disable throttling, but this may impact server performance.',
            required: false,
